...
const loadingCassettes = ref(true);
...
const loadCassettes = async () => {
  loadingCassettes.value = true;
  try {
    const { request } = useApi();
    cassettes.value = await request('/admin/cassettes');
  } catch (error) {
    console.error('Ошибка загрузки кассет:', error);
    alert('Не удалось загрузить кассеты.');
  } finally {
    loadingCassettes.value = false;
  }
};
...
const saveCassette = async () => {
  ...
  try {
    ...
    alert('Не удалось сохранить кассету.');
  } finally {
    saving.value = false;
  }
};
...
const deleteCassette = async (id: number) => {
  ...
  try {
    ...
    alert('Не удалось удалить кассету.');
  } finally {
...
<template>
  <!-- Единое окно с одной неоновой рамкой -->
  <DecoratorRgbBorder
      class="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-900 p-4 md:p-6 relative overflow-hidden"
  >
    <!-- Одна неоновая рамка по краю -->
    <div class="absolute inset-0 border-2 border-cyan-400 rounded-lg pointer-events-none"></div>

    <!-- Весь контент -->
    <div class="relative z-10">
      <!-- Header -->
      <div class="flex justify-between items-center mb-8">
        <h1 class="text-2xl md:text-3xl font-orbitron text-cyan-300">📁 Каталог кассет</h1>
        <button
            @click="logout"
      ...
      <div class="mb-8">
        <button
            ...
            class="px-5 py-2.5 bg-emerald-900/60 hover:bg-emerald-800/70 rounded-md font-orbitron border border-emerald-500 transition flex items-center gap-2"
        >
          <span>➕</span> Добавить кассету
        </button>
      </div>

      <!-- Form -->
      <div v-if="showForm" class="mb-10 p-6 bg-transparent rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
          ...
          <input
              ...
              class="w-full p-3 bg-gray-900 border border-yellow-600 rounded text-yellow-100 placeholder:text-yellow-500/60 focus:outline-none focus:ring-1 focus:ring-yellow-500"
              required
          />
          ...
          <input
              ...
              class="w-full p-3 bg-gray-900 border border-pink-600 rounded text-pink-100 placeholder:text-pink-500/60 focus:outline-none focus:ring-1 focus:ring-pink-500"
          />
          ...
          <div class="md:col-span-2">
            <input
                ...
                class="w-full p-3 bg-gray-900 border border-purple-600 rounded text-purple-100 placeholder:text-purple-500/60 focus:outline-none focus:ring-1 focus:ring-purple-500"
            />
          </div>
        ...
        <div class="pt-4 border-t border-gray-700">
          ...
          <div class="space-y-4">
            ...
            <textarea
                ...
                class="w-full p-3 bg-gray-900 border border-emerald-600 rounded text-emerald-100 placeholder:text-emerald-500/60 focus:outline-none focus:ring-1 focus:ring-emerald-500"
            ></textarea>
        ...
        <div class="mt-5 flex gap-3">
          <button
              :disabled="saving"
              @click="saveCassette"
              class="px-5 py-2.5 bg-cyan-900/70 hover:bg-cyan-800/80 rounded font-orbitron border border-cyan-500 transition flex items-center gap-2"
          >
      ...
      <div>
        ...
        <!-- Loading -->
        <div v-if="loadingCassettes" class="py-8 flex justify-center">
          <div class="w-6 h-6 border-2 border-cyan-500 border-t-transparent rounded-full animate-spin"></div>
        </div>

        <!-- Empty -->
        <div v-else-if="cassettes.length === 0" class="text-gray-400 italic py-4">
        ...
        <div v-else class="space-y-4">
          <div
              ...
              :key="cassette.id"
              class="p-5 bg-gray-900/50 rounded-lg border border-gray-700 flex flex-col md:flex-row justify-between items-start gap-6 md:gap-8"
          >
            <!-- Images -->
            <div class="flex-shrink-0 flex gap-4">
              <div class="flex flex-col gap-2">
                <img
                    ...
                    alt="Front cover"
                    class="w-20 h-20 object-cover rounded border border-cyan-600/50"
                ...
                <img
                    ...
                    alt="Back cover"
                    class="w-20 h-20 object-cover rounded border border-purple-600/50"
            ...
            <div class="flex-grow min-w-0">
              <h3 class="font-orbitron text-xl text-cyan-200">{{ cassette.title }}</h3>
              <p class="text-sm text-yellow-300">{{ cassette.brand }} • {{ cassette.year }}</p>
              <p v-if="cassette.description" class="mt-2 text-gray-300 text-base line-clamp-3">
            ...
            <div class="flex gap-2 self-start md:self-center">
              <button
                  :disabled="deletingId === cassette.id"
                  @click="deleteCassette(cassette.id)"
                  class="p-2 text-red-400 hover:text-red-200 hover:bg-red-900/30 rounded transition disabled:opacity-50 disabled:cursor-not-allowed"
                  :title="deletingId === cassette.id ? 'Удаление...' : 'Удалить'"
              >
                <svg v-if="deletingId !== cassette.id" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path fill-rule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z" clip-rule="evenodd" />
                </svg>
                <div v-else class="w-5 h-5 border-2 border-red-400 border-t-transparent rounded-full animate-spin"></div>
              </button>
              <button
